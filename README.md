<!-- ████████████████████████████████████████████████████████████ -->
<!--                  IH SAFY — CYBERPUNK README                 -->
<!-- ████████████████████████████████████████████████████████████ -->

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Share+Tech+Mono&weight=700&size=20&duration=3000&pause=1000&color=00FF41&center=true&vCenter=true&width=750&lines=%5BBOOT%5D+Initializing+Neural+Core...+%E2%9C%93;%5BONLINE%5D+AI+%26+Machine+Learning+Engineer;%5BACTIVE%5D+Computer+Vision+%7C+Deep+Learning;%5BREADY%5D+LLM+Systems+%7C+AI+Automation;%5BLOCALE%5D+Dhaka%2C+Bangladesh+%F0%9F%87%A7%F0%9F%87%A9)](https://git.io/typing-svg)

</div>

<br>

<div align="center">

```
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║    ██╗██╗  ██╗    ███████╗ █████╗ ███████╗██╗   ██╗            ║
║    ██║██║  ██║    ██╔════╝██╔══██╗██╔════╝╚██╗ ██╔╝            ║
║    ██║███████║    ███████╗███████║█████╗   ╚████╔╝             ║
║    ██║██╔══██║    ╚════██║██╔══██║██╔══╝    ╚██╔╝              ║
║    ██║██║  ██║    ███████║██║  ██║██║        ██║               ║
║    ╚═╝╚═╝  ╚═╝    ╚══════╝╚═╝  ╚═╝╚═╝        ╚═╝               ║
║                                                                  ║
║              AI  ·  MACHINE LEARNING  ·  COMPUTER VISION        ║
╚══════════════════════════════════════════════════════════════════╝
```

</div>

<div align="center">

![](https://img.shields.io/badge/───────────────────%20NEURAL%20LINK%20ESTABLISHED%20───────────────────-00FF41?style=flat-square&labelColor=0d0d0d&color=0d0d0d)

</div>

<div align="center">

![AI Engineer](https://img.shields.io/badge/⚡_AI_ENGINEER-0d0d0d?style=for-the-badge&color=0d0d0d&logoColor=00FF41)
![CV](https://img.shields.io/badge/🔍_COMPUTER_VISION-0d0d0d?style=for-the-badge&color=0d0d0d)
![DL](https://img.shields.io/badge/🧠_DEEP_LEARNING-0d0d0d?style=for-the-badge&color=0d0d0d)
![LLM](https://img.shields.io/badge/🤖_LLM_SYSTEMS-0d0d0d?style=for-the-badge&color=0d0d0d)

</div>

<div align="center">

![Views](https://komarev.com/ghpvc/?username=ihsafy&label=UPLINK+COUNT&color=00FF41&style=flat-square&labelColor=0d0d0d)
&nbsp;
![Followers](https://img.shields.io/github/followers/ihsafy?style=flat-square&color=FF003C&labelColor=0d0d0d&label=FOLLOWERS)
&nbsp;
![Stars](https://img.shields.io/github/stars/ihsafy?style=flat-square&color=00BFFF&labelColor=0d0d0d&label=STARS)

</div>

---

## `⟫ 01 // OPERATOR PROFILE`

```
┌─────────────────────────────────────────────────────────────┐
│                   SYSTEM IDENTITY FILE                      │
├──────────────────┬──────────────────────────────────────────┤
│  HANDLE          │  IH SAFY                                 │
│  DESIGNATION     │  AI & Machine Learning Engineer          │
│  NODE            │  Dhaka, Bangladesh 🇧🇩                  │
│  INSTITUTION     │  East West University                    │
│  DEGREE          │  B.Sc. in Computer Science & Eng.       │
│  STATUS          │  ONLINE ⚡                               │
├──────────────────┴──────────────────────────────────────────┤
│  CORE DIRECTIVES                                            │
│  ▸ Artificial Intelligence & Automation                     │
│  ▸ Deep Learning & Neural Architectures                     │
│  ▸ Computer Vision & Object Detection                       │
│  ▸ LLM Engineering & RAG Systems                            │
│  ▸ Intelligent System Design                                │
├─────────────────────────────────────────────────────────────┤
│  MISSION  »  Build AI that solves real-world problems       │
└─────────────────────────────────────────────────────────────┘
```

---

## `⟫ 02 // TECH ARSENAL`

<div align="center">

**`[ LANGUAGES ]`**

![Python](https://img.shields.io/badge/Python-0d0d0d?style=for-the-badge&logo=python&logoColor=00FF41)
![Java](https://img.shields.io/badge/Java-0d0d0d?style=for-the-badge&logo=openjdk&logoColor=FF003C)
![C](https://img.shields.io/badge/C-0d0d0d?style=for-the-badge&logo=c&logoColor=00BFFF)
![PHP](https://img.shields.io/badge/PHP-0d0d0d?style=for-the-badge&logo=php&logoColor=9B59B6)
![JavaScript](https://img.shields.io/badge/JavaScript-0d0d0d?style=for-the-badge&logo=javascript&logoColor=FF6B00)

**`[ AI / ML FRAMEWORKS ]`**

![TensorFlow](https://img.shields.io/badge/TensorFlow-0d0d0d?style=for-the-badge&logo=tensorflow&logoColor=FF6B00)
![PyTorch](https://img.shields.io/badge/PyTorch-0d0d0d?style=for-the-badge&logo=pytorch&logoColor=FF003C)
![OpenCV](https://img.shields.io/badge/OpenCV-0d0d0d?style=for-the-badge&logo=opencv&logoColor=00FF41)
![Scikit-Learn](https://img.shields.io/badge/Scikit--Learn-0d0d0d?style=for-the-badge&logo=scikitlearn&logoColor=F7931E)
![Pandas](https://img.shields.io/badge/Pandas-0d0d0d?style=for-the-badge&logo=pandas&logoColor=00BFFF)
![NumPy](https://img.shields.io/badge/NumPy-0d0d0d?style=for-the-badge&logo=numpy&logoColor=9B59B6)
![Jupyter](https://img.shields.io/badge/Jupyter-0d0d0d?style=for-the-badge&logo=jupyter&logoColor=FF6B00)

**`[ WEB INTERFACE ]`**

![HTML5](https://img.shields.io/badge/HTML5-0d0d0d?style=for-the-badge&logo=html5&logoColor=FF003C)
![CSS3](https://img.shields.io/badge/CSS3-0d0d0d?style=for-the-badge&logo=css3&logoColor=00BFFF)
![Bootstrap](https://img.shields.io/badge/Bootstrap-0d0d0d?style=for-the-badge&logo=bootstrap&logoColor=9B59B6)
![MySQL](https://img.shields.io/badge/MySQL-0d0d0d?style=for-the-badge&logo=mysql&logoColor=00FF41)

**`[ TOOLS & OPS ]`**

![Git](https://img.shields.io/badge/Git-0d0d0d?style=for-the-badge&logo=git&logoColor=FF003C)
![GitHub](https://img.shields.io/badge/GitHub-0d0d0d?style=for-the-badge&logo=github&logoColor=ffffff)
![VSCode](https://img.shields.io/badge/VS_Code-0d0d0d?style=for-the-badge&logo=visualstudiocode&logoColor=00BFFF)
![Linux](https://img.shields.io/badge/Linux-0d0d0d?style=for-the-badge&logo=linux&logoColor=FF6B00)

</div>

---

## `⟫ 03 // SPECIALIZATION MATRIX`

```
╔═══════════════════════════════════════════════════════════════════╗
║                      DOMAIN REGISTRY                             ║
╠══════════════════════╦═══════════════════════╦═══════════════════╣
║  SECTOR              ║  ARSENAL              ║  DIRECTIVE        ║
╠══════════════════════╬═══════════════════════╬═══════════════════╣
║  🧠 Machine Learning ║  Scikit-Learn/Pandas  ║  Prediction       ║
║  👁  Computer Vision ║  OpenCV / CNN / YOLO  ║  Detection        ║
║  ⚡ Deep Learning    ║  TensorFlow/PyTorch   ║  Neural Networks  ║
║  🤖 LLM Engineering  ║  Prompt Eng. / RAG    ║  Automation       ║
║  🌐 Backend Systems  ║  PHP / MySQL          ║  Web Infra        ║
╚══════════════════════╩═══════════════════════╩═══════════════════╝
```

---

## `⟫ 04 // SKILL CALIBRATION`

```
╔═════════════════════════════════════════════════════════════════╗
║                    PROFICIENCY SCAN REPORT                      ║
╠═══════════════════════╦══════════════════════════╦═════════════╣
║  MODULE               ║  NEURAL LOAD             ║  GRADE      ║
╠═══════════════════════╬══════════════════════════╬═════════════╣
║  Machine Learning     ║  ████████████████████░░  ║  85%        ║
║  Computer Vision      ║  ██████████████████░░░░  ║  78%        ║
║  Deep Learning        ║  ████████████████░░░░░░  ║  70%        ║
║  LLM Engineering      ║  ██████████████░░░░░░░░  ║  62%        ║
║  Backend Dev          ║  ████████████░░░░░░░░░░  ║  55%        ║
╚═══════════════════════╩══════════════════════════╩═════════════╝
```

---

## `⟫ 05 // MISSION ARCHIVE`

```
╔══════════════════════════════════════════════════════════════════╗
║              ░▒▓  CLASSIFIED OPERATION FILES  ▓▒░               ║
╚══════════════════════════════════════════════════════════════════╝

┌──────────────────────────────────────────────────────────────────┐
│  OPERATION: MINDSCANNER           [MEDICAL_AI] [DEEP_LEARNING]  │
├──────────────────────────────────────────────────────────────────┤
│  Alzheimer's MRI Multi-Class Classification via CNN.             │
│  Medical imaging analysis with intelligent detection pipeline.   │
│  ▸ Architecture : Convolutional Neural Network                   │
│  ▸ Domain       : Healthcare / Neuroradiology                    │
│  ▸ Status       : ✓ DEPLOYED                                     │
└──────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────┐
│  OPERATION: RORA LUXE             [FULL_STACK] [E-COMMERCE]     │
├──────────────────────────────────────────────────────────────────┤
│  Full-stack luxury marketplace with secure backend system.       │
│  Responsive UI/UX with complete product management pipeline.     │
│  ▸ Stack        : PHP + MySQL                                    │
│  ▸ Domain       : E-Commerce / Retail                            │
│  ▸ Status       : ✓ DEPLOYED                                     │
└──────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────┐
│  OPERATION: BOOK SEEKERS          [SMART_PLATFORM] [LIBRARY]    │
├──────────────────────────────────────────────────────────────────┤
│  Intelligent library platform with smart search & AI recs.       │
│  Full database integration for seamless book management.         │
│  ▸ Stack        : PHP + MySQL                                    │
│  ▸ Domain       : Education / Information Retrieval              │
│  ▸ Status       : ✓ DEPLOYED                                     │
└──────────────────────────────────────────────────────────────────┘

┌──────────────────────────────────────────────────────────────────┐
│  OPERATION: EWU FOOD HUB          [CAMPUS_TECH] [REAL_TIME]     │
├──────────────────────────────────────────────────────────────────┤
│  Campus food listing platform built for student experience.      │
│  Real-time interface with student-focused UX design.             │
│  ▸ Stack        : Web Technologies                               │
│  ▸ Domain       : Campus Life / Food-Tech                        │
│  ▸ Status       : ✓ DEPLOYED                                     │
└──────────────────────────────────────────────────────────────────┘
```

---

## `⟫ 06 // OBJECTIVE TREE`

```
ROOT@SAFY:~$ cat roadmap.log
──────────────────────────────────────────────────────────────────
  [✓]  BUILD    »  AI & ML Foundations ................. COMPLETE
  [✓]  FORGE    »  Real-World Project Portfolio ........ COMPLETE
  [✓]  UNLOCK   »  AI Automation Pipelines ............. COMPLETE
  [⟳]  MASTER   »  Computer Vision Architecture ........ RUNNING
  [⟳]  ADVANCE  »  Deep Learning Systems ............... RUNNING
  [⟳]  DEPLOY   »  Production-Grade AI Infra ........... RUNNING
  [ ]  PUBLISH  »  Research Papers ..................... QUEUED
  [ ]  MERGE    »  Open Source Contributions ........... QUEUED
  [ ]  ACHIEVE  »  Professional AI Engineer Status ...... QUEUED
──────────────────────────────────────────────────────────────────
ROOT@SAFY:~$ █
```

---

## `⟫ 07 // GITHUB ANALYTICS`

```
╔══════════════════════════════════════════════════════════════════╗
║                   REPOSITORY SYSTEM REPORT                       ║
╠══════════════════════════════════════════════════════════════════╣
║                                                                  ║
║   TOTAL CONTRIBUTIONS  ›  140        Mar 2022 — Present          ║
║   CURRENT STREAK       ›  2 days     May 17 — May 18             ║
║   LONGEST STREAK       ›  4 days     Jan 17 — Jan 20, 2025       ║
║                                                                  ║
╠══════════════════════════════════════════════════════════════════╣
║   TOP LANGUAGES                                                  ║
║   ▸ Python        ████████████████████░░░░  Primary              ║
║   ▸ PHP           ████████████████░░░░░░░░  Secondary            ║
║   ▸ JavaScript    ████████████░░░░░░░░░░░░  Tertiary             ║
║   ▸ HTML / CSS    █████████░░░░░░░░░░░░░░░  Interface            ║
╠══════════════════════════════════════════════════════════════════╣
║   COMMIT FREQUENCY  ›  Active                                    ║
║   PUBLIC REPOS      ›  Growing                                   ║
║   ACCOUNT SINCE     ›  Mar 13, 2022                              ║
╚══════════════════════════════════════════════════════════════════╝
```

<div align="center">

![Commits](https://img.shields.io/badge/TOTAL_COMMITS-140-00FF41?style=for-the-badge&labelColor=0d0d0d)
![Streak](https://img.shields.io/badge/CURRENT_STREAK-2_days-FF003C?style=for-the-badge&labelColor=0d0d0d)
![Longest](https://img.shields.io/badge/LONGEST_STREAK-4_days-00BFFF?style=for-the-badge&labelColor=0d0d0d)
![Since](https://img.shields.io/badge/ACTIVE_SINCE-Mar_2022-FF6B00?style=for-the-badge&labelColor=0d0d0d)

</div>

---

## `⟫ 08 // UPLINK PROTOCOL`

<div align="center">

```
╔══════════════════════════════════════════════════════╗
║          ESTABLISH SECURE COMMUNICATION LINK         ║
╚══════════════════════════════════════════════════════╝
```

[![Gmail](https://img.shields.io/badge/SECURE_MAIL_%7C_ihsafy2k21%40gmail.com-0d0d0d?style=for-the-badge&logo=gmail&logoColor=FF003C&labelColor=0d0d0d)](mailto:ihsafy2k21@gmail.com)
&nbsp;&nbsp;
[![GitHub](https://img.shields.io/badge/GITHUB_NODE_%7C_ihsafy-0d0d0d?style=for-the-badge&logo=github&logoColor=00FF41&labelColor=0d0d0d)](https://github.com/ihsafy)

</div>

---

## `⟫ 09 // CORE DIRECTIVE`

<div align="center">

```
╔══════════════════════════════════════════════════════════════════╗
║                                                                  ║
║   "Artificial Intelligence is not just technology.              ║
║                                                                  ║
║    It is the power to build systems that amplify human           ║
║    potential, eliminate friction, and architect a smarter        ║
║    world — one model at a time."                                 ║
║                                                                  ║
║                                              — IH SAFY           ║
╚══════════════════════════════════════════════════════════════════╝
```

</div>

---

<div align="center">

[![Typing SVG](https://readme-typing-svg.demolab.com?font=Share+Tech+Mono&size=15&duration=3500&pause=1500&color=00FF41&center=true&vCenter=true&width=700&lines=%5BSYSTEM%5D+Session+terminated.+Neural+signature+logged.;%5BGRID%5D+Thank+you+for+visiting.+See+you+in+the+network.+%F0%9F%94%A5)](https://git.io/typing-svg)

```
▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
             CONNECTION CLOSED  //  SEE YOU IN THE GRID
▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓
```

</div>
